  ...
  return (
    <div className="pt-16">
      ...
      <section id="offers" className="container mx-auto px-4 my-10 space-y-6">
        <h3 className="text-3xl font-extrabold text-blue-600 to-indigo-600 mb-6">Offres récentes</h3>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {currentOffers.map((offer) => (
                <div key={offer.id} className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300">
            ...
            <div className="flex justify-center items-center gap-3 mt-8">
              <button onClick={handlePrev} disabled={currentPage === 1} className={`w-10 h-10 flex items-center justify-center rounded-full border transition-all duration-300 ${currentPage === 1 ? "text-gray-400 border-gray-300 cursor-not-allowed" : "text-blue-600 border-blue-600 hover:bg-blue-50"}`} >{"<"}</button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                <button key={num} onClick={() => setCurrentPage(num)} className={`w-10 h-10 flex items-center justify-center rounded-full border transition-all duration-300 ${currentPage === num ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg" : "text-blue-600 border-blue-600 hover:bg-blue-50"}`}>{num}</button>
              ))}
              <button onClick={handleNext} disabled={currentPage === totalPages} className={`w-10 h-10 flex items-center justify-center rounded-full border transition-all duration-300 ${currentPage === totalPages ? "text-gray-400 border-gray-300 cursor-not-allowed" : "text-blue-600 border-blue-600 hover:bg-blue-50"}`}>{">"}</button>
            </div>
          </>
        ) : (
      ...
      {/* À propos */}
      <section id="about" className="bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-16 px-6 md:px-12">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div>
           <h3 className="text-3xl font-extrabold text-blue-600 to-indigo-600 mb-6">A propos</h3>
            <p className="text-gray-700 leading-relaxed text-lg space-y-4">
              Bienvenue sur <span className="font-bold text-blue-600 to-indigo-600">JobSpace</span> votre site de recherche d'emploi au Sénégal !<br/>
          ...
          <div className="flex justify-center">
            <img src="/JobSpace.png" alt="Illustration à propos" className="rounded-2xl shadow-xl w-full max-w-md object-cover"/>
          </div>
        </div>
      </section>

      {/* Contact */}
      <section id="contact" className="py-16 px-6 md:px-12 bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-blue-600 to-indigo-600 mb-4">Contactez-nous</h2>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">Une question ? Un projet ? N'hésitez pas à nous contacter, nous serons ravis de vous aider.</p>
          </div>
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-gray-100 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
              <form onSubmit={(e) => { e.preventDefault(); alert("Merci, votre message a bien été envoyé."); (e.target as HTMLFormElement).reset(); }} className="space-y-6">
                <input name="nom" type="text" required placeholder="Nom complet" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"/>
                <input name="email" type="email" required placeholder="votre@email.com" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"/>
                <textarea name="message" rows={5} required placeholder="Votre message" className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200 resize-none"></textarea>
                <button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold py-4 px-6 rounded-xl hover:from-blue-700 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl">Envoyer le message</button>
              </form>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                ...
                <a href="tel:+221770000000" className="text-purple-600 hover:text-purple-800 font-medium">+221 77 000 00 00</a>
              </div>
            </div>
          </div>
        </div>
